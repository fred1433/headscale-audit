<!-- Generated by lab/audit.sh. Do not edit by hand. -->
# Report of the laboratory run
...
Three findings are expected here and say something about the tool:

1. **HS-001** the laboratory serves plain http on the loopback, because a
   local lab cannot hold a public certificate. The audit reports it rather
   than making an exception for itself.
2. **HS-008** the SQLite database is mode 0644. Nobody chose that: it is how
   headscale 0.29.3 creates the file, next to a noise key it correctly
   creates 0600. That is exactly the kind of thing an audit is for.
3. **HS-064** an expired pre-auth key is left over from
   `lab/rollout_test.sh`, which issues a key valid for one second to prove
   that the enrolment script fails with an explanation.
...
| low | HS-064 | Expired credential left in place |
...
### HS-064 [low] Expired credential left in place

- **Expected**: expired credentials are deleted
- **Read**: 1 pre-auth key(s): [key id 5 (user lab)]
...
- Pre-auth keys and API keys: HS-060, HS-061, HS-062, HS-063
